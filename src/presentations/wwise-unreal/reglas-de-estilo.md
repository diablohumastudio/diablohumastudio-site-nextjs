...
- El arco de la clase: Portada → la biblioteca (analogía de disco/RAM/caché) → morph a hardware → el restaurante (qué es un servidor: petición/proceso/respuesta) → morph a cliente/servidor → cada servidor sirve lo suyo (web/FTP/base de datos) → la computadora servidor (hardware para servir 24/7, sin gráfica) → el arranque del juego en un celular (el juego corre en RAM con el init cargado y soundbank_menus sube del disco al vuelo) → (por definir; mantener la progresión de lo general a lo específico).
