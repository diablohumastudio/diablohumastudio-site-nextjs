...
Cuando una secuencia es *el mismo diagrama con el mismo h2* que se va construyendo pieza a pieza, no se hace con varios slides `=` sino con un **slide con pasos**: `label` recibe un array (un rótulo por paso) y el contenido lee el paso actual con `useSlideStep()` (mecánica en `docs/learn-courses-and-slides.md`). Cada `→` añade una pieza sin remontar el slide, así que solo la pieza nueva lleva `morphIn` y las anteriores quedan estáticas. Los slides `=` quedan para el morph a *otra idea* (biblioteca → computador, restaurante → cliente/servidor).
...
- La analogía: estanterías = disco, la mesa larga con libros cerrados en las puntas = RAM, los libros abiertos del centro = caché, las dos caritas sentadas arriba y abajo de la mesa (fuera de ella) = CPU. Las tres flechas codifican velocidad con su largo: levantarse a la estantería (lento) > alcanzar la punta de la mesa (rápido) > leer el libro abierto (al instante, flechas cortas hacia cada carita). Los iconos son propios (`LibroCerrado`, `LibroAbierto`, `Cara`), sin librerías externas.
...
- El slide "cliente-servidor" (z="=") repite ese mismo patrón desde "el restaurante" (z=3): geometría compartida en `EscenaPeticion` (cliente a la izquierda, servidor grande a la derecha, dos flechas), y un solo componente de rótulos `RotulosPeticion` que recibe el objeto de textos de cada dominio (tú → PC cliente, cocina → PC servidor, flechas → petición/respuesta). El servidor lista tres pasos numerados dentro de su caja: verificar el plato / revisar y traer ingredientes / preparar → validar usuario / buscar recursos / armar la respuesta. No hay caja de recursos externa.
- El slide "servidores" (z=4) lleva a la derecha de cada caja dos ejemplos reales (Apache · Nginx, FileZilla Server · vsftpd, PostgreSQL · MySQL).
- El slide "hardware" (z=5) compara dos máquinas lado a lado con el componente `Maquina`: la computadora servidor (tarjetas de red y fuente en ámbar; tarjeta de video punteada, "inexistente") y la PC de diseño gráfico (tarjeta de video en ámbar; red y fuente básicas). Las flechas de peticiones/respuestas entran por arriba del servidor.
- El arranque es un solo slide con pasos (z=6, rótulos "arranque" / "pide" / "busca" / "sube"): `FiguraArranque` lee el paso con `useSlideStep()` y dibuja `EscenaArranque({ paso })` — cada `→` añade un paso y anima solo la pieza nueva con `morphIn` (las anteriores quedan estáticas). Un paso nuevo es una entrada más en el array de `label`, un `aria`/`caption` más en el diccionario y su pieza en `EscenaArranque`. El disco muestra EL JUEGO con sus SoundBanks como cajas individuales (soundbank_init, soundbank_menus, soundbank_nivel_1); la RAM arranca con game engine, escena main, sound engine y soundbank_init (cajas sueltas, sin contenedor) y una posición libre punteada. Pasos: ① escena main → sound engine («suena el menú»), ② sound engine → soundbank_menus en el disco, ③ el banco sube en codo a la posición libre y aparece soundbank_menus con `morphPulse` (0.6s después de la flecha) — nivel_1 se queda en disco. Pendiente: pasos siguientes donde aparece soundbank_nivel_1 en RAM y desaparece soundbank_menus.
