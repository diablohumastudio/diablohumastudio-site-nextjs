...
import { editorDict } from './el-editor-wwise.dict';
...
/* Los subtítulos (sub) salen del diccionario, en este mismo orden. */
type ViewUtil = { x: number; nombre: string; delay: string };
const VIEWS_UTILES: ViewUtil[] = [
  { x: 140, nombre: 'Project Explorer', delay: '0s' },
  { x: 600, nombre: 'Transport Control', delay: '0.6s' },
  { x: 370, nombre: 'Soundcaster', delay: '1.2s' },
];

type EditorItem = { x: number; y: number; nombre: string; sub: string };
type EditorPosicion = Omit<EditorItem, 'sub'>;
const EDITORS_PROPIEDADES: EditorPosicion[] = [
  { x: 70, y: 182, nombre: 'RTPC' },
  { x: 345, y: 182, nombre: 'States' },
  { x: 620, y: 182, nombre: 'Effects' },
  { x: 207, y: 254, nombre: 'Stingers' },
  { x: 482, y: 254, nombre: 'Metadata' },
];

const PROPERTY_EDITOR_TABS: string[] = ['General', 'Routing', 'Conversion', 'Positioning', 'Advanced', '…'];

const LAYOUTS_FABRICA_NOMBRES: string[] = [
  'Designer · F5',
  'Profiler · F6',
  'SoundBank · F7',
  'Mixer · F8',
...
function MarcoViews({ titulo, viewsDirectos }: { titulo: string; viewsDirectos: string }) {
  return (
    <>
      <rect x="30" y="16" width="880" height="328" rx="10" fill="#1d2026" stroke="currentColor" strokeOpacity=".35" />
      <text x="48" y="46" fontSize="11.5" fill="currentColor" opacity=".65" letterSpacing="2">{titulo}</text>
...
function CajaCategoria({ x, nombre, submenu, atenuada = false }: { x: number; nombre: string; submenu: string; atenuada?: boolean }) {
  return (
    <g>
...
function MarcoEditors({ titulo }: { titulo: string }) {
  return (
    <>
      <rect x="40" y="30" width="860" height="290" rx="10" fill="#1d2026" stroke="#f2a33c" strokeOpacity=".8" />
      <text x="58" y="60" fontSize="11.5" fill="#f2a33c" letterSpacing="2">{titulo}</text>
    </>
  );
}
...
function CajaPropertyEditor({ titulo }: { titulo: string }) {
  ...
  return (
    <g>
      ...
      <text x="470" y="104" fontSize="12" fill="currentColor" textAnchor="middle">{titulo}</text>
...
export default function ElEditorWwise() {
  const t = useT(editorDict);
  const editorsPropiedades: EditorItem[] = EDITORS_PROPIEDADES.map((editor, index) => ({ ...editor, sub: t.porPropiedad.subs[index] }));
  ...
  return (
    <Deck name={t.name} context={t.context}>
      ...
      <Slide z="1" label={t.labels.views}>
        <div className={s.eyebrow}>{t.views.eyebrow}</div>
        <h2>{t.views.title}</h2>
        <figure>
          <svg viewBox="0 0 940 360" role="img" aria-label={t.views.aria}>
            <MarcoViews titulo={t.marco.views} viewsDirectos={t.marco.viewsDirectos} />
            <CajaCategoria x={60} nombre="Editors" submenu={t.marco.submenu} />
            <CajaCategoria x={340} nombre="Profiler" submenu={t.marco.submenu} />
            <CajaCategoria x={620} nombre="Utilities" submenu={t.marco.submenu} />
          ...
          <figcaption>{t.views.caption}</figcaption>
        </figure>
      </Slide>

      <Slide z="2" label={t.labels.losUtiles}>
        <div className={s.eyebrow}>{t.losUtiles.eyebrow}</div>
        <h2>{t.losUtiles.title}</h2>
        <figure>
          <svg viewBox="0 0 940 360" role="img" aria-label={t.losUtiles.aria}>
            <MarcoViews titulo={t.marco.views} viewsDirectos={t.marco.viewsDirectos} />
            <g className={s.morphPulse} style={{ animationDelay: '1.8s' }}>
              <CajaCategoria x={60} nombre="Editors" submenu={t.marco.submenu} />
            </g>
            <CajaCategoria x={340} nombre="Profiler" submenu={t.marco.submenu} atenuada />
            <CajaCategoria x={620} nombre="Utilities" submenu={t.marco.submenu} atenuada />
            {VIEWS_DIRECTOS_FILA1.map(([x, nombre]) => (
            ...
            {VIEWS_UTILES.map(({ x, nombre, delay }, index) => (
              <g key={nombre}>
                ...
                <g className={s.morphPulse} style={{ animationDelay: delay }}>
                  <g className={s.morphIn} style={{ animationDelay: delay }}>
                    ...
                    <text x={x + 100} y="298" fontSize="10.5" fill="currentColor" opacity=".6" textAnchor="middle">{t.losUtiles.subs[index]}</text>
                  </g>
                </g>
              </g>
            ))}
          </svg>
          <figcaption>{t.losUtiles.caption}</figcaption>
        </figure>
      </Slide>

      <Slide z="3" label={t.labels.porPropiedad}>
        <div className={s.eyebrow}>{t.porPropiedad.eyebrow}</div>
        <h2>{t.porPropiedad.title}</h2>
        <figure>
          <svg viewBox="0 0 940 350" role="img" aria-label={t.porPropiedad.aria}>
            <g className={s.morphGlide} style={{ '--morph-from': 'translate(48px, 59px) scale(0.3)' } as CSSProperties}>
              <MarcoEditors titulo={t.marco.editors} />
            </g>
            <g className={s.morphIn} style={{ animationDelay: '0.6s' }}>
              <text className={s.svgSans} x="882" y="60" fontSize="11" fill="#63b6a4" textAnchor="end">
                {t.porPropiedad.propiedadesDeCualquierObjeto}
              </text>
              <CajaPropertyEditor titulo={t.porPropiedad.propertyEditor} />
              {editorsPropiedades.map((editor) => (
                <CajaEditor key={editor.nombre} width={250} height={60} {...editor} />
              ))}
            </g>
          </svg>
          <figcaption>{t.porPropiedad.caption}</figcaption>
        </figure>
      </Slide>

      <Slide z="4" label={t.labels.porObjeto}>
        <div className={s.eyebrow}>{t.porObjeto.eyebrow}</div>
        <h2>{t.porObjeto.title}</h2>
        <figure>
          <svg viewBox="0 0 940 350" role="img" aria-label={t.porObjeto.aria}>
            <MarcoEditors titulo={t.marco.editors} />
            <g className={s.morphOut}>
              <text className={s.svgSans} x="882" y="60" fontSize="11" fill="#63b6a4" textAnchor="end">
                {t.porPropiedad.propiedadesDeCualquierObjeto}
          ...
          <figcaption>{t.porObjeto.caption}</figcaption>
        </figure>
      </Slide>

      <Slide z="5" label={t.labels.layouts}>
        <div className={s.eyebrow}>{t.layouts.eyebrow}</div>
        <h2>{t.layouts.title}</h2>
        <figure>
          <svg viewBox="0 0 940 310" role="img" aria-label={t.layouts.aria}>
            ...
            <text className={s.svgSans} x="700" y="296" fontSize="11" fill="currentColor" opacity=".5" textAnchor="middle">
              {t.layouts.otroLayout}
            </text>
          </svg>
          <figcaption>{t.layouts.caption}</figcaption>
        </figure>
      </Slide>

      <Slide z="6" label={t.labels.deFabrica}>
        <div className={s.eyebrow}>{t.deFabrica.eyebrow}</div>
        <h2>{t.deFabrica.title}</h2>
        <table className={s.plain}>
          <thead>
            <tr>
              <th>{t.deFabrica.thLayout}</th>
              <th>{t.deFabrica.thParaQue}</th>
            </tr>
          </thead>
          <tbody>
            {LAYOUTS_FABRICA_NOMBRES.map((nombre, index) => (
              <tr key={nombre}>
                <td>{nombre}</td>
                <td>{t.deFabrica.propositos[index]}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Slide z="7" label={t.labels.designer}>
        <div className={s.eyebrow}>{t.designer.eyebrow}</div>
        <h2>{t.designer.title}</h2>
        <figure>
          <svg viewBox="0 0 940 470" role="img" aria-label={t.designer.aria}>
            <defs>
            ...
            <text className={s.svgSans} x="458" y="34" fontSize="11" fill="#63b6a4" textAnchor="middle">
              {t.designer.todosLosElementosAbiertos}
            </text>
            ...
            <text x="160" y="164" fontSize="12" fill="#f2a33c" textAnchor="middle">{t.designer.projectExplorer}</text>
            <text x="160" y="184" fontSize="10.5" fill="currentColor" opacity=".6" textAnchor="middle">{t.designer.navegarLaJerarquia}</text>
            <text x="160" y="200" fontSize="10.5" fill="currentColor" opacity=".6" textAnchor="middle">{t.designer.deObjetosWwise}</text>
            <rect x="60" y="282" width="200" height="70" rx="8" fill="#232730" stroke="currentColor" strokeOpacity=".35" />
            <text x="160" y="314" fontSize="11.5" fill="currentColor" textAnchor="middle">{t.designer.contextualHelp}</text>
            <text x="160" y="332" fontSize="10.5" fill="currentColor" opacity=".6" textAnchor="middle">{t.designer.ayudaDeLoQueApuntas}</text>
            <rect x="60" y="364" width="200" height="66" rx="8" fill="#232730" stroke="#f2a33c" strokeOpacity=".8" />
            <text x="160" y="393" fontSize="11.5" fill="currentColor" textAnchor="middle">{t.designer.transportControl}</text>
            <text x="160" y="411" fontSize="10.5" fill="currentColor" opacity=".6" textAnchor="middle">{t.designer.escuchar}</text>
            ...
            <text className={s.svgSans} x="458" y="152" fontSize="10" fill="#63b6a4" textAnchor="middle">
              {t.designer.pestanasDependientes}
            </text>
            <text className={s.svgSans} x="458" y="300" fontSize="11" fill="currentColor" opacity=".4" textAnchor="middle">
              {t.designer.loQueTengasSeleccionado}
            </text>

            <rect x="656" y="80" width="224" height="350" rx="8" fill="#232730" stroke="#f2a33c" strokeOpacity=".8" />
            <text x="768" y="108" fontSize="12" fill="currentColor" textAnchor="middle">{t.designer.propertyEditor}</text>
            <text x="768" y="128" fontSize="10.5" fill="currentColor" opacity=".6" textAnchor="middle">{t.designer.lasPropiedadesDelObjeto}</text>
          </svg>
          <figcaption>{t.designer.caption}</figcaption>
        </figure>
      </Slide>
    </Deck>
  );
